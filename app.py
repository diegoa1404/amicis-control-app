...
def main():
    ...
    if 'usuario_logado' not in st.session_state:
        ...
        st.set_page_config(
            ...
            # Estado inicial da barra lateral: "expanded", "collapsed", ou "auto"
        ...
        if st.button("Entrar"):
            client = get_googlesheet_client1()

            # Carregar dados da planilha
            users_data = load_sheet(
                CREDENTIALS_SPREADSHEET_ID).sheet1.get_all_records()
            ...
            if usuario:
                ...
                st.error(
                    "Login ou senha incorretos, ou o seu cadastro está inativo.")
    else:
        ...
        if 'Ano' not in df.columns:
            # Supondo que a coluna de data seja 'Data'
        ...
        with st.sidebar:
            ...
            selected = option_menu(
                ...
                icons=["house", "bar-chart",
                       "currency-exchange", "clipboard-data"],
                menu_icon="cast",
                default_index=0,
            )
        ...
        with filtros_container:
            ...
            with col3:
                # Filtro de Ativo
                ativos_disponiveis = ['Todos'] + \
                    sorted(df_filtrado['Ativo'].unique())
            ...
            with col4:
                # Filtro de Setup
                setups_disponiveis = ['Todos'] + \
                    sorted(df_filtrado["Setup"].unique())
                setup_selecionado = st.selectbox(
                    "Escolha o Setup", setups_disponiveis)
                if setup_selecionado != 'Todos':
                    df_filtrado = df_filtrado[df_filtrado["Setup"]
                                              == setup_selecionado]
        ...
        def format_metric(label, value, bg_color="rgb(51, 51, 51)", font_color="rgb(255, 255, 255)",
            ...
            if label == "Lucro/Prejuízo (R$)":
                formatted_value = f"R$ {value:,.2f}".replace(
                    ",", "X").replace(".", ",").replace("X", ".")
                ...
                # Formato genérico caso não se encaixe nas categorias
        ...
        if selected == "Dashboard":
            ...
            avg_gain = df_filtrado[df_filtrado["Gain/Loss"]
                                   == "Gain"]["R$"].mean()
            avg_loss = df_filtrado[df_filtrado["Gain/Loss"]
                                   == "Loss"]["R$"].mean()
            risco_retorno = avg_gain / abs(avg_loss) if avg_loss != 0 else 0
            breakeven = (1 / (1 + risco_retorno) *
                         100) if risco_retorno > 0 else 0
            edge = taxa_acerto - breakeven if breakeven > 0 else 0
            total_gain = df_filtrado[df_filtrado["Gain/Loss"]
                                     == "Gain"]["R$"].sum()
            total_loss = df_filtrado[df_filtrado["Gain/Loss"]
                                     == "Loss"]["R$"].sum()
            fator_lucro = total_gain / \
                abs(total_loss) if total_loss != 0 else 0
            ...
            with col4:
                st.markdown(format_metric("Breakeven (%)", breakeven),
                            unsafe_allow_html=True)
            with col5:
                st.markdown(format_metric("Edge (%)", edge),
                            unsafe_allow_html=True)
            with col6:
            ...
            with col1:
                st.markdown(
                    "<h3 style='font-weight:bold;'>Número de Trades</h3>", unsafe_allow_html=True)
                ...
                fig_trades = go.Figure(go.Bar(
                    ...
                    textposition=[get_textposition(
                        v) for v in trade_df_filtrado["Número de Trades"]],
                    # Negrito nos dados
                ...
                fig_pizza = go.Figure(go.Pie(
                    # Rótulos (Gain, Loss, Draw)
                    labels=trade_df_filtrado_pizza["Tipo"],
                    # Valores de cada categoria
                    values=trade_df_filtrado_pizza["Número de Trades"],
                    # Cores personalizadas
                    marker=dict(colors=trade_df_filtrado_pizza["Cor"]),
                    textinfo='percent+label',  # Mostrar porcentagem e rótulo
                    insidetextorientation='radial',  # Texto dentro da pizza
                    # Negrito apenas nos valores
                    textfont=dict(
                        size=14, family="Arial, sans-serif", weight="bold")
                ))
                ...
                with col2:
                    st.markdown("<h3>Média de Gain e Loss</h3>",
                                unsafe_allow_html=True)
                    ...
                    gain_loss_data = {
                        ...
                        "Valor": [
                            ...
                            round(abs(avg_loss), 2) if not pd.isna(
                                avg_loss) else 0  # Absoluto para Loss
                        ],
                        # Verde para Gain, Vermelho para Loss
                        "Cor": ["green", "red"],
                    }
                    ...
                    fig_gain_loss = go.Figure(go.Bar(
                        ...
                        text=[format_value(v)
                              for v in gain_loss_df_filtrado["Valor"]],
                        textposition=[get_textposition(
                            v) for v in gain_loss_df_filtrado["Valor"]],
            ...
            with col2:
                ...
                fig_pizza_gain_loss = go.Figure(go.Pie(
                    # Rótulos (Média Gain, Média Loss)
                    labels=gain_loss_df_filtrado["Tipo"],
                    # Valores de cada categoria
                    values=gain_loss_df_filtrado["Valor"],
                    # Cores personalizadas
                    marker=dict(colors=gain_loss_df_filtrado["Cor"]),
                    textinfo='percent+label',  # Mostrar porcentagem e rótulo
                    insidetextorientation='radial',  # Texto dentro da pizza
                    # Negrito apenas nos valores
                    textfont=dict(
                ...
                with col3:
                    st.markdown("<h3>Melhor e Pior Trade</h3>",
                                unsafe_allow_html=True)
                    ...
                    fig_best_worst = go.Figure(go.Bar(
                        ...
                        # Formatando com separadores
                        text=[format_value(v)
                              for v in best_worst_df_filtrado["Valor"]],
                        textposition=[get_textposition(
                            v) for v in best_worst_df_filtrado["Valor"]],
                    ...
                    fig_best_worst.update_layout(
                        ...
                        # Fundo do papel também escuro
                        paper_bgcolor='rgb(40, 40, 40)',
                        # Texto em branco e em negrito
                        font=dict(color="white",
                                  family="Arial, sans-serif", weight="bold"),
                        xaxis=dict(
                            color="white",
                            showgrid=False,
                            # Título do eixo X em negrito
                            title_font=dict(
                                size=14, family="Arial, sans-serif", weight="bold")
                        ),
                        yaxis=dict(
            ...
            with col3:
                ...
                best_worst_data = {
                    ...
                    "Valor": [
                        ...
                        round(abs(worst_trade), 2) if not pd.isna(
                            worst_trade) else 0  # Número absoluto para o pior trade
                    ],
                    # Verde para Melhor, Vermelho para Pior
                    "Cor": ["green", "red"],
                }
                ...
                fig_pizza_best_worst = go.Figure(go.Pie(
                    # Rótulos (Melhor Trade, Pior Trade)
                    labels=best_worst_df_filtrado["Tipo"],
                    # Valores de cada categoria
                    values=best_worst_df_filtrado["Valor"],
                    # Cores personalizadas
                    marker=dict(colors=best_worst_df_filtrado["Cor"]),
                    textinfo='percent+label',  # Mostrar porcentagem e rótulo
                    insidetextorientation='radial',  # Texto dentro da pizza
                    # Negrito apenas nos valores
                    textfont=dict(
                ...
                with col4:
                    # Gráfico de Patrimônio
                    st.markdown("<h3>Gráfico de Patrimônio</h3>",
                                unsafe_allow_html=True)
                    ...
                    fig_patrimonio.add_trace(go.Scatter(
                        ...
                        # Eixo Y: Resultado acumulado
                        y=df_filtrado["Resultado Acumulado"],
                        ...
                        fillcolor="rgba(255, 0, 0, 0.3)" if df_filtrado[
                            "Resultado Acumulado"].iloc[-1] < 0 else "rgba(0, 255, 0, 0.3)",
                        # Cor da linha
                        line=dict(
                            color="rgb(255, 0, 0)" if df_filtrado["Resultado Acumulado"].iloc[-1] < 0 else "rgb(0, 255, 0)"),
                    ...
                    fig_patrimonio.update_layout(
                        ...
                        xaxis=dict(
                            ...
                            # Título do eixo X em negrito
                            title_font=dict(
                                size=16, family="Arial, sans-serif", weight='bold'),
                            color="white",
                            showgrid=False,
                            # Valores do eixo X em negrito
                            tickfont=dict(
                                size=14, family="Arial, sans-serif", weight='bold')
                        ),
                        yaxis=dict(
            ...
            with col1:
                st.markdown("<h3>Resultado por Setup</h3>",
                            unsafe_allow_html=True)

                # Calculando o resultado por setup
                result_por_setup = df_filtrado.groupby(
                    "Setup")["R$"].sum().reset_index()
                ...
                fig_result_por_setup = go.Figure(go.Bar(
                    # Eixo Y para barras horizontais
                    y=result_por_setup["Setup"],
                    ...
                    textfont=dict(
                        ...
                        color=[text_color(cor)
                               for cor in result_por_setup["Cor"]]
                    ),
            ...
            with col2:
                st.markdown("<h3>Resultado por Ativo</h3>",
                            unsafe_allow_html=True)

                # Calculando o resultado por ativo
                result_por_ativo = df_filtrado.groupby(
                    "Ativo")["R$"].sum().reset_index()
            ...
            with col1:
                ...
                st.markdown("<h3>Resultado por Operação</h3>",
                            unsafe_allow_html=True)
                ...
                fig_resultado = go.Figure(go.Bar(
                    ...
                    # Eixo Y: Resultado da operação (R$)
                    y=df_filtrado_trades["R$"],
                ...
                fig_resultado.update_layout(
                    ...
                    xaxis=dict(
                        ...
                        color="white",
                        showgrid=False,
                        # Remover os rótulos (ticks) do eixo X
            ...
            with col2:
                ...
                fig_patrimonio.add_trace(go.Scatter(
                    ...
                    mode="lines",  # Linha contínua
                    fill="tozeroy",  # Preencher para o eixo Y
                    # Cor cinza para o preenchimento
                    fillcolor="rgba(128,128,128,0.3)",
                    line=dict(color="rgb(105, 105, 105)",
                              width=2),  # Linha cinza escuro
                    name="Resultado Acumulado",
                ))
        ...
        # Resultado Mês
            ...
            def calcular_metricas(df_filtrado):
                ...
                df_filtrado['Mês'] = df_filtrado['Data'].dt.month.apply(
                    lambda x: meses_pt[x - 1])
                ...
                resultados_mensais = df_filtrado.groupby('Mês').agg(
                    ...
                    total_ganho=(
                        'R$', lambda x: x[df_filtrado['Gain/Loss'] == 'Gain'].sum()),
                    total_perda=(
                        'R$', lambda x: x[df_filtrado['Gain/Loss'] == 'Loss'].sum())
                )

                # Cálculos adicionais
                resultados_mensais['taxa_acerto'] = (resultados_mensais['num_gain'] / (
                    resultados_mensais['num_gain'] + resultados_mensais['num_loss'])) * 100
                resultados_mensais['média_ganho'] = df_filtrado[df_filtrado['Gain/Loss']
                                                                == 'Gain'].groupby('Mês')['R$'].mean()
                resultados_mensais['média_perda'] = df_filtrado[df_filtrado['Gain/Loss']
                                                                == 'Loss'].groupby('Mês')['R$'].mean()
                resultados_mensais['risco_retorno'] = resultados_mensais['média_ganho'] / abs(
                    resultados_mensais['média_perda'])
                resultados_mensais['breakeven'] = 100 / \
                    (1 + resultados_mensais['risco_retorno'])
                resultados_mensais['fator_lucro'] = resultados_mensais['total_ganho'] / abs(
                    resultados_mensais['total_perda'])
                resultados_mensais['edge'] = resultados_mensais['taxa_acerto'] - \
                    resultados_mensais['breakeven']

                # Calcular o Resultado Bruto (Total Ganho - Total Perda)
                resultados_mensais['resultado_bruto'] = resultados_mensais['total_ganho'] + \
                    resultados_mensais['total_perda']
                ...
                resultados_mensais = resultados_mensais.dropna(
                    subset=['quantidade_trades'])
                ...
                resultados_mensais['quantidade_trades'] = resultados_mensais['quantidade_trades'].astype(
                    int)
                resultados_mensais['num_gain'] = resultados_mensais['num_gain'].astype(
                    int)
                resultados_mensais['num_loss'] = resultados_mensais['num_loss'].astype(
                    int)
                resultados_mensais['num_draw'] = resultados_mensais['num_draw'].astype(
                    int)

                resultados_mensais['taxa_acerto'] = resultados_mensais['taxa_acerto'].map(
                    lambda x: f"{x:.2f}%")
                resultados_mensais['fator_lucro'] = resultados_mensais['fator_lucro'].map(
                    lambda x: f"{x:.2f}")
                resultados_mensais['edge'] = resultados_mensais['edge'].map(
                    lambda x: f"{x:.2f}%")
                resultados_mensais['total_ganho'] = resultados_mensais['total_ganho'].map(
                    lambda x: f"R$ {x:,.2f}")
                resultados_mensais['total_perda'] = resultados_mensais['total_perda'].map(
                    lambda x: f"R$ {x:,.2f}")
                resultados_mensais['resultado_bruto'] = resultados_mensais['resultado_bruto'].map(
                    lambda x: f"R$ {x:,.2f}")
                resultados_mensais['risco_retorno'] = resultados_mensais['risco_retorno'].map(
                    lambda x: f"{x:.2f}")
                resultados_mensais['média_ganho'] = resultados_mensais['média_ganho'].map(
                    lambda x: f"R$ {x:,.2f}")
                resultados_mensais['média_perda'] = resultados_mensais['média_perda'].map(
                    lambda x: f"R$ {x:,.2f}")
                resultados_mensais['breakeven'] = resultados_mensais['breakeven'].map(
                    lambda x: f"{x:.2f}%")
            ...
            def aplicar_estilos(df_filtrado):
                ...
                # Novas colunas para estilo condicional
                colunas_condicionais = ['resultado_bruto', 'edge']
                ...
                styled_df_filtrado = df_filtrado.style.apply(
                    lambda coluna: [estilo_condicional(
                        valor, coluna.name) for valor in coluna],
                    axis=0
                )
            ...
            def main():
                ...
                styled_table = aplicar_estilos(
                    resultados_mensais.set_index('Mês'))  # Define o Mês como índice
            ...
            with col1:
                ...
                def calcular_metricas(df):
                    ...
                    resultados_mensais = df.groupby(['Num_Mês', 'Mês']).agg(
                        ...
                        num_draw=('Gain/Loss', lambda x: (x == 'Draw').sum()),
                        total_gain=(
                            'R$', lambda x: x[df['Gain/Loss'] == 'Gain'].sum()),
                        total_loss=(
                            'R$', lambda x: x[df['Gain/Loss'] == 'Loss'].sum()),
                    ).reset_index()
                ...
                resultados_mensais = resultados_mensais.sort_values(
                    by='Num_Mês')

                # Gráfico do número de trades por mês
                st.markdown("<h3>Número de Trades por Mês</h3>",
                            unsafe_allow_html=True)

                fig_trades_por_mes = go.Figure(go.Bar(
                    x=resultados_mensais['Mês'],  # Meses no eixo X
                    # Quantidade de trades no eixo Y
                    y=resultados_mensais['quantidade_trades'],
                    # Exibe o número de trades nas barras
                    text=resultados_mensais['quantidade_trades'],
                    textposition='outside',  # Posição do texto fora da barra
                    # Texto das barras em negrito
                ...
                fig_trades_por_mes.update_layout(
                    ...
                    font=dict(color="white", family="Arial, sans-serif",
                              weight="bold"),  # Fonte geral em negrito
                    xaxis=dict(
                        title="Mês",  # Título do eixo X
                        color="white",
                        showgrid=False,
                        # Título do eixo em negrito
                        title_font=dict(
                            size=14, family="Arial, sans-serif", weight="bold"),
                    ),
                    yaxis=dict(
            ...
            with col2:
                ...
                def calcular_metricas(df):
                    ...
                    df['Mês'] = df['Mês_Num'].apply(
                        lambda x: meses_pt[x - 1])  # Nome do mês traduzido
                    ...
                    resultados_mensais['Resultado'] = resultados_mensais['total_gain'] + \
                        resultados_mensais['total_loss']
                ...
                st.markdown("<h3>Resultado Bruto por Mês</h3>",
                            unsafe_allow_html=True)

                # Definindo a cor das barras (verde para resultado positivo, vermelho para negativo)
                resultados_mensais['Cor'] = resultados_mensais['Resultado'].apply(
                    lambda x: 'green' if x > 0 else 'red')

                # Função para formatar os valores como moeda
                def format_currency(value):
                    formatted_value = f"R$ {abs(value):,.2f}".replace(
                        ",", "#").replace(".", ",").replace("#", ".")
                ...
                fig_resultado_por_mes = go.Figure(go.Bar(
                    x=resultados_mensais['Mês'],  # Meses no eixo X
                    # Resultado bruto no eixo Y
                    y=resultados_mensais['Resultado'],
                    # Exibe o resultado formatado nas barras
                    text=[format_currency(v)
                          for v in resultados_mensais['Resultado']],
                    textposition='outside',  # Posição do texto fora da barra
                    textfont=dict(size=12, family="Arial, sans-serif",
                                  color="white", weight="bold"),  # Texto em negrito
                    # Cor das barras (verde ou vermelho)
                    marker=dict(color=resultados_mensais['Cor']),
                ))
                ...
                fig_resultado_por_mes.update_layout(
                    ...
                    paper_bgcolor='rgb(40, 40, 40)',  # Fundo do papel escuro
                    font=dict(color="white", family="Arial, sans-serif",
                              weight="bold"),  # Texto do gráfico em negrito
                    xaxis=dict(
                        title="Mês",  # Título do eixo X
                        color="white",
                        # Título do eixo em negrito
            ...
            with col1:
                ...
                def calcular_metricas(df):
                    ...
                    resultados_mensais['media_gain'] = resultados_mensais['total_gain'] / \
                        resultados_mensais['num_gain']
                    resultados_mensais['media_loss'] = resultados_mensais['total_loss'].abs(
                    ) / resultados_mensais['num_loss']

                    # Calculando risco/retorno (média ganho / média perda)
                    resultados_mensais['risco_retorno'] = resultados_mensais['media_gain'] / \
                        resultados_mensais['media_loss']
                ...
                st.markdown("<h3>Risco/Retorno por Mês</h3>",
                            unsafe_allow_html=True)
                ...
                fig_risco_retorno = go.Figure(go.Bar(
                    x=resultados_mensais['Mês'],  # Meses no eixo X
                    # Risco/Retorno no eixo Y
                    y=resultados_mensais['risco_retorno'],
                    # Exibe o risco/retorno formatado nas barras
                    text=[format_risco_retorno(
                        v) for v in resultados_mensais['risco_retorno']],
                    textposition='outside',  # Posição do texto fora da barra
                    # Texto das barras em negrito
                    textfont=dict(size=12, family="Arial, sans-serif",
                                  color="white", weight="bold"),
            ...
            with col2:
                ...
                resultados_mensais['Breakeven (%)'] = (
                    1 / (1 + resultados_mensais['risco_retorno'])) * 100

                # Gráfico do breakeven por mês
                st.markdown("<h3>Breakeven (%) por Mês</h3>",
                            unsafe_allow_html=True)
                ...
                fig_breakeven_por_mes = go.Figure(go.Bar(
                    x=resultados_mensais['Mês'],  # Meses no eixo X
                    # Breakeven percentual no eixo Y
                    y=resultados_mensais['Breakeven (%)'],
                    # Exibe o breakeven formatado nas barras
                    text=[format_percent(v)
                          for v in resultados_mensais['Breakeven (%)']],
                    textposition='outside',  # Posição do texto fora da barra
                    # Texto das barras em negrito
                    textfont=dict(size=12, family="Arial, sans-serif",
                                  color="white", weight="bold"),
        ...
        # Resultado por Ativo
            ...
            def calcular_metricas(df_filtrado):
                ...
                # Cálculos adicionais
                resultados_ativos['taxa_acerto'] = (resultados_ativos['num_gain'] / (
                    resultados_ativos['num_gain'] + resultados_ativos['num_loss'])) * 100
                resultados_ativos['média_ganho'] = df_filtrado[df_filtrado['Gain/Loss']
                                                               == 'Gain'].groupby('Ativo')['R$'].mean()
                resultados_ativos['média_perda'] = df_filtrado[df_filtrado['Gain/Loss']
                                                               == 'Loss'].groupby('Ativo')['R$'].mean()
                resultados_ativos['risco_retorno'] = resultados_ativos['média_ganho'] / abs(
                    resultados_ativos['média_perda'])
                resultados_ativos['breakeven'] = 100 / \
                    (1 + resultados_ativos['risco_retorno'])
                resultados_ativos['fator_lucro'] = resultados_ativos['total_ganho'] / \
                    abs(resultados_ativos['total_perda'])
                resultados_ativos['edge'] = resultados_ativos['taxa_acerto'] - \
                    resultados_ativos['breakeven']

                # Calcular o Resultado Bruto (Total Ganho - Total Perda)
                resultados_ativos['resultado_bruto'] = resultados_ativos['total_ganho'] + \
                    resultados_ativos['total_perda']
                ...
                # Formatação dos valores
                resultados_ativos['quantidade_trades'] = resultados_ativos['quantidade_trades'].astype(
                    int)
                resultados_ativos['num_gain'] = resultados_ativos['num_gain'].astype(
                    int)
                resultados_ativos['num_loss'] = resultados_ativos['num_loss'].astype(
                    int)
                resultados_ativos['num_draw'] = resultados_ativos['num_draw'].astype(
                    int)

                resultados_ativos['taxa_acerto'] = resultados_ativos['taxa_acerto'].map(
                    lambda x: f"{x:.2f}%")
                resultados_ativos['fator_lucro'] = resultados_ativos['fator_lucro'].map(
                    lambda x: f"{x:.2f}")
                resultados_ativos['edge'] = resultados_ativos['edge'].map(
                    lambda x: f"{x:.2f}%")
                resultados_ativos['total_ganho'] = resultados_ativos['total_ganho'].map(
                    lambda x: f"R$ {x:,.2f}")
                resultados_ativos['total_perda'] = resultados_ativos['total_perda'].map(
                    lambda x: f"R$ {x:,.2f}")
                resultados_ativos['resultado_bruto'] = resultados_ativos['resultado_bruto'].map(
                    lambda x: f"R$ {x:,.2f}")
                resultados_ativos['risco_retorno'] = resultados_ativos['risco_retorno'].map(
                    lambda x: f"{x:.2f}")
                resultados_ativos['média_ganho'] = resultados_ativos['média_ganho'].map(
                    lambda x: f"R$ {x:,.2f}")
                resultados_ativos['média_perda'] = resultados_ativos['média_perda'].map(
                    lambda x: f"R$ {x:,.2f}")
                resultados_ativos['breakeven'] = resultados_ativos['breakeven'].map(
                    lambda x: f"{x:.2f}%")
            ...
            def main():
                ...
                styled_table = aplicar_estilos(resultados_ativos.set_index(
                    'Ativo'))  # Define o Ativo como índice
                st.dataframe(styled_table)
            ...
            with col1:
                ...
                resultados_por_ativo = resultados_por_ativo.sort_values(
                    by='quantidade_trades', ascending=False)

                # Gráfico do número de trades por ativo
                st.markdown("<h3>Número de Trades por Ativo</h3>",
                            unsafe_allow_html=True)
                ...
                fig_trades_por_ativo = go.Figure(go.Bar(
                    ...
                    # Quantidade de trades no eixo Y
                    y=resultados_por_ativo['quantidade_trades'],
                    # Exibe o número de trades nas barras
                    text=resultados_por_ativo['quantidade_trades'],
                    textposition='outside',  # Posição do texto fora da barra
                    # Texto das barras em negrito
                    textfont=dict(size=12, family="Arial, sans-serif",
                                  color="white", weight="bold"),
            ...
            with col2:
                ...
                def calcular_metricas_por_ativo(df):
                    ...
                    resultados_por_ativo['Resultado'] = resultados_por_ativo['total_gain'] + \
                        resultados_por_ativo['total_loss']
                ...
                resultados_por_ativo = resultados_por_ativo.sort_values(
                    by='Resultado', ascending=False)

                # Gráfico do resultado bruto por ativo
                st.markdown("<h3>Resultado Bruto por Ativo</h3>",
                            unsafe_allow_html=True)
                ...
                resultados_por_ativo['Resultado Formatado'] = resultados_por_ativo['Resultado'].apply(
                    formatar_moeda)

                # Definindo a cor das barras (verde para resultado positivo, vermelho para negativo)
                resultados_por_ativo['Cor'] = resultados_por_ativo['Resultado'].apply(
                    lambda x: 'green' if x > 0 else 'red')
                ...
                fig_resultado_por_ativo = go.Figure(go.Bar(
                    x=resultados_por_ativo['Ativo'],  # Ativos no eixo X
                    # Resultado bruto no eixo Y
                    y=resultados_por_ativo['Resultado'],
                    # Exibe o resultado formatado como moeda
                    text=resultados_por_ativo['Resultado Formatado'],
                    textposition='outside',  # Posição do texto fora da barra
                    # Texto das barras em negrito
                    textfont=dict(size=12, family="Arial, sans-serif",
                                  color="white", weight="bold"),
                    # Cor das barras (verde ou vermelho)
                    marker=dict(color=resultados_por_ativo['Cor']),
                ))
            ...
            with col1:
                ...
                def calcular_metricas_por_ativo(df):
                    ...
                    # Calculando médias de ganho e perda
                    resultados_por_ativo['media_gain'] = resultados_por_ativo['total_gain'] / \
                        resultados_por_ativo['num_gain']
                    resultados_por_ativo['media_loss'] = resultados_por_ativo['total_loss'].abs(
                    ) / resultados_por_ativo['num_loss']

                    # Calculando risco/retorno (média ganho / média perda)
                    resultados_por_ativo['risco_retorno'] = resultados_por_ativo['media_gain'] / \
                        resultados_por_ativo['media_loss']
                ...
                st.markdown("<h3>Risco/Retorno por Ativo</h3>",
                            unsafe_allow_html=True)
                ...
                fig_risco_retorno = go.Figure(go.Bar(
                    x=resultados_por_ativo['Ativo'],  # Ativos no eixo X
                    # Risco/Retorno no eixo Y
                    y=resultados_por_ativo['risco_retorno'],
                    # Exibe o risco/retorno formatado nas barras
                    text=resultados_por_ativo['risco_retorno_formatado'],
                    textposition='outside',  # Posição do texto fora da barra
                    textfont=dict(size=12, family="Arial, sans-serif",
                                  color="white", weight="bold"),  # Texto em negrito
                    marker=dict(color='orange'),  # Cor das barras
            ...
            with col2:
                ...
                # Calculando o breakeven com base no risco/retorno já existente
                resultados_por_ativo['Breakeven (%)'] = (
                    1 / (1 + resultados_por_ativo['risco_retorno'])) * 100

                # Gráfico do breakeven por ativo
                st.markdown("<h3>Breakeven (%) por Ativo</h3>",
                            unsafe_allow_html=True)
                ...
                fig_breakeven_por_ativo = go.Figure(go.Bar(
                    x=resultados_por_ativo['Ativo'],  # Ativos no eixo X
                    # Breakeven percentual no eixo Y
                    y=resultados_por_ativo['Breakeven (%)'],
                    # Exibe o breakeven formatado nas barras
                    text=resultados_por_ativo['breakeven_formatado'],
                    textposition='outside',  # Posição do texto fora da barra
                    textfont=dict(size=12, family="Arial, sans-serif",
                                  color="white", weight="bold"),  # Texto em negrito
                    marker=dict(color='rgb(102, 194, 255)'),  # Cor das barras
        ...
        # Resultado por Setup
            ...
            def calcular_metricas_por_setup(df_filtrado):
                ...
                # Cálculos adicionais
                resultados_setup['taxa_acerto'] = (resultados_setup['num_gain'] / (
                    resultados_setup['num_gain'] + resultados_setup['num_loss'])) * 100
                resultados_setup['média_ganho'] = df_filtrado[df_filtrado['Gain/Loss']
                                                              == 'Gain'].groupby('Setup')['R$'].mean()
                resultados_setup['média_perda'] = df_filtrado[df_filtrado['Gain/Loss']
                                                              == 'Loss'].groupby('Setup')['R$'].mean()
                resultados_setup['risco_retorno'] = resultados_setup['média_ganho'] / \
                    abs(resultados_setup['média_perda'])
                resultados_setup['breakeven'] = 100 / \
                    (1 + resultados_setup['risco_retorno'])
                resultados_setup['fator_lucro'] = resultados_setup['total_ganho'] / \
                    abs(resultados_setup['total_perda'])
                resultados_setup['edge'] = resultados_setup['taxa_acerto'] - \
                    resultados_setup['breakeven']

                # Calcular o Resultado Bruto (Total Ganho - Total Perda)
                resultados_setup['resultado_bruto'] = resultados_setup['total_ganho'] + \
                    resultados_setup['total_perda']

                # Ordenar pelo Setup
                resultados_setup = resultados_setup.sort_index()

                # Filtrar para remover os setups sem dados
                resultados_setup = resultados_setup.dropna(
                    subset=['quantidade_trades'])
                ...
                # Formatação dos valores
                resultados_setup['quantidade_trades'] = resultados_setup['quantidade_trades'].astype(
                    int)
                resultados_setup['num_gain'] = resultados_setup['num_gain'].astype(
                    int)
                resultados_setup['num_loss'] = resultados_setup['num_loss'].astype(
                    int)
                resultados_setup['num_draw'] = resultados_setup['num_draw'].astype(
                    int)

                resultados_setup['taxa_acerto'] = resultados_setup['taxa_acerto'].map(
                    lambda x: f"{x:.2f}%")
                resultados_setup['fator_lucro'] = resultados_setup['fator_lucro'].map(
                    lambda x: f"{x:.2f}")
                resultados_setup['edge'] = resultados_setup['edge'].map(
                    lambda x: f"{x:.2f}%")
                resultados_setup['total_ganho'] = resultados_setup['total_ganho'].map(
                    lambda x: f"R$ {x:,.2f}")
                resultados_setup['total_perda'] = resultados_setup['total_perda'].map(
                    lambda x: f"R$ {x:,.2f}")
                resultados_setup['resultado_bruto'] = resultados_setup['resultado_bruto'].map(
                    lambda x: f"R$ {x:,.2f}")
                resultados_setup['risco_retorno'] = resultados_setup['risco_retorno'].map(
                    lambda x: f"{x:.2f}")
                resultados_setup['média_ganho'] = resultados_setup['média_ganho'].map(
                    lambda x: f"R$ {x:,.2f}")
                resultados_setup['média_perda'] = resultados_setup['média_perda'].map(
                    lambda x: f"R$ {x:,.2f}")
                resultados_setup['breakeven'] = resultados_setup['breakeven'].map(
                    lambda x: f"{x:.2f}%")
            ...
            def main():
                ...
                styled_table = aplicar_estilos(resultados_setup.set_index(
                    'Setup'))  # Define o Setup como índice
                st.dataframe(styled_table)
            ...
            with col1:
                ...
                # Ordenando os resultados por quantidade de trades (opcional)
                resultados_por_setup = resultados_por_setup.sort_values(
                    by='quantidade_trades', ascending=False)

                # Gráfico do número de trades por setup
                st.markdown("<h3><b>Número de Trades por Setup</b></h3>",
                            unsafe_allow_html=True)  # Título em negrito

                # Criando o gráfico de barras
                fig_trades_por_setup = go.Figure(go.Bar(
                    x=resultados_por_setup['Setup'],  # Setups no eixo X
                    # Quantidade de trades no eixo Y
                    y=resultados_por_setup['quantidade_trades'],
                    # Exibe o número de trades nas barras
                    text=resultados_por_setup['quantidade_trades'],
                    textposition='outside',  # Posição do texto fora da barra
                    # Texto em negrito nas barras
                    textfont=dict(size=12, family="Arial, sans-serif",
                                  color="white", weight="bold"),
                    marker=dict(color='rgb(169, 169, 169)'),  # Cor das barras
                ))
                ...
                fig_trades_por_setup.update_layout(
                    plot_bgcolor='rgb(40, 40, 40)',  # Fundo escuro
                    paper_bgcolor='rgb(40, 40, 40)',  # Fundo do papel escuro
                    # Cor e negrito para o texto do gráfico
                    font=dict(color="white",
                              family="Arial, sans-serif", weight="bold"),
                    xaxis=dict(
                ...
                with col2:
                    ...
                    def calcular_metricas_por_setup(df):
                        ...
                        resultados_por_setup = df.groupby('Setup').agg(
                            quantidade_trades=('R$', 'size'),
                            num_gain=(
                                'Gain/Loss', lambda x: (x == 'Gain').sum()),
                            num_loss=(
                                'Gain/Loss', lambda x: (x == 'Loss').sum()),
                            num_draw=(
                                'Gain/Loss', lambda x: (x == 'Draw').sum()),
                            total_gain=(
                                'R$', lambda x: x[df['Gain/Loss'] == 'Gain'].sum()),
                            total_loss=(
                                'R$', lambda x: x[df['Gain/Loss'] == 'Loss'].sum()),
                        ...
                        resultados_por_setup['Resultado'] = resultados_por_setup['total_gain'] + \
                            resultados_por_setup['total_loss']

                        return resultados_por_setup

                    # Chamando a função para calcular as métricas por setup
                    resultados_por_setup = calcular_metricas_por_setup(
                        df_filtrado)
                    ...
                    st.markdown("<h3><b>Resultado Bruto por Setup</b></h3>",
                                unsafe_allow_html=True)  # Título em negrito

                    # Definindo a cor das barras (verde para resultado positivo, vermelho para negativo)
                    resultados_por_setup['Cor'] = resultados_por_setup['Resultado'].apply(
                        lambda x: 'green' if x > 0 else 'red')
                    ...
                    fig_resultado_por_setup = go.Figure(go.Bar(
                        x=resultados_por_setup['Setup'],  # Setups no eixo X
                        # Resultado bruto no eixo Y
                        y=resultados_por_setup['Resultado'],
                        text=resultados_por_setup['Resultado'].apply(lambda x: f"R$ {x:,.2f}".replace(
                            ",", ";").replace(".", ",").replace(";", ".")),  # Formato R$ 623,00
                        textposition='outside',  # Posição do texto fora da barra
                        # Texto em negrito nas barras
                        textfont=dict(
                            size=12, family="Arial, sans-serif", color="white", weight="bold"),
                        # Cor das barras (verde ou vermelho)
                        marker=dict(color=resultados_por_setup['Cor']),
                    ))

                    # Ajustando o layout para o gráfico
                    fig_resultado_por_setup.update_layout(
                        plot_bgcolor='rgb(40, 40, 40)',  # Fundo escuro
                        # Fundo do papel escuro
                        paper_bgcolor='rgb(40, 40, 40)',
                        # Cor e negrito para o texto do gráfico
                        font=dict(color="white",
                                  family="Arial, sans-serif", weight="bold"),
            ...
            with col1:
                ...
                def calcular_metricas_por_setup(df):
                    ...
                    # Calculando médias de ganho e perda
                    resultados_por_setup['media_gain'] = resultados_por_setup['total_gain'] / \
                        resultados_por_setup['num_gain']
                    resultados_por_setup['media_loss'] = resultados_por_setup['total_loss'].abs(
                    ) / resultados_por_setup['num_loss']

                    # Calculando risco/retorno (média ganho / média perda)
                    resultados_por_setup['risco_retorno'] = resultados_por_setup['media_gain'] / \
                        resultados_por_setup['media_loss']
                ...
                st.markdown("<h3><b>Risco/Retorno por Setup</b></h3>",
                            unsafe_allow_html=True)  # Título em negrito
                ...
                fig_risco_retorno = go.Figure(go.Bar(
                    ...
                    # Risco/Retorno no eixo Y
                    y=resultados_por_setup['risco_retorno'],
                    text=resultados_por_setup['risco_retorno'].apply(lambda x: f"{x:,.2f}".replace(
                        ",", ";").replace(".", ",").replace(";", ".")),  # Formato 2,21
                    textposition='outside',  # Posição do texto fora da barra
                    # Texto em negrito nas barras
                    textfont=dict(size=12, family="Arial, sans-serif",
                                  color="white", weight="bold"),
                ...
                with col2:
                    ...
                    # Calculando o breakeven com base no risco/retorno já existente
                    resultados_por_setup['Breakeven (%)'] = (
                        1 / (1 + resultados_por_setup['risco_retorno'])) * 100

                    # Gráfico do breakeven por setup
                    st.markdown("<h3><b>Breakeven (%) por Setup</b></h3>",
                                unsafe_allow_html=True)  # Título em negrito

                    # Formatando os valores para exibir como 31,44%
                    resultados_por_setup['Breakeven (%)'] = resultados_por_setup['Breakeven (%)'].apply(
                        lambda x: f"{x:,.2f}".replace(",", ";").replace(".", ",").replace(";", "."))

                    fig_breakeven_por_setup = go.Figure(go.Bar(
                        x=resultados_por_setup['Setup'],  # Setup no eixo X
                        # Breakeven percentual no eixo Y
                        y=resultados_por_setup['Breakeven (%)'],
                        # Exibe o breakeven percentual nas barras
                        text=resultados_por_setup['Breakeven (%)'],
                        textposition='outside',  # Posição do texto fora da barra
                        # Texto em negrito nas barras
                        textfont=dict(
                            size=12, family="Arial, sans-serif", color="white", weight="bold"),
                        # Cor das barras
                        marker=dict(color='rgb(102, 194, 255)'),
                    ))
